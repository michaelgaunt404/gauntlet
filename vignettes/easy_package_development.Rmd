...
This function identifies unqualified function calls and replaces them with the correct `[package]::[function]` notation. It's useful to run this function to ensure that all function calls are properly qualified.

The function indicates which lines of your code contained unqualified function calls by appending `#!!!! LINE CHANGED` to each line.   
...
Finally, we'll use the `checkPkg_make_importFrom_fnctn_statments` function. This function generates @importFrom statements for each package detected in your script, which is essential for proper documentation.    

We will run in on our newly edited function `process_mtcars.R` with the full `package::function` declarations.    
